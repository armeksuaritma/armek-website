...
function bindEngagementTracking(){
 ...
 window.addEventListener("scroll",()=>{
  ...
  if(percent>=50&&!sent50){sent50=true;trackEvent("scroll_depth",{percent_scrolled:50,page_location:location.href})}
  if(percent>=90&&!sent90){sent90=true;trackEvent("scroll_depth",{percent_scrolled:90,page_location:location.href})}
 },{passive:true});
 document.addEventListener("click",event=>{
  const link=event.target.closest("a");if(!link)return;
  const href=link.href||"";
  if(href&&link.hostname&&link.hostname!==location.hostname&&!/^https:\/\/wa\.me\//i.test(href)){
   trackEvent("outbound_click",{link_url:href,link_text:(link.textContent||"").trim(),page_location:location.href});
  }
 },{capture:true});
}
...
function renderProducts(items=[]){
 ...
 const draw=cat=>{
  ...
  root.innerHTML=list.map((p,i)=>`<article class="product-card reveal" data-product-index="${items.indexOf(p)}">${p.badge?`<span class="product-badge">${p.badge}</span>`:""}<a class="product-image" href="/detay.html?tip=urun&slug=${encodeURIComponent(p.slug||seoSlug(p.name))}" aria-label="${p.name||"Ürün"} detayını aç"><img src="${p.image||productPhotos(p)[0]||"/armek-logo.jpg"}" alt="${p.name||"Ürün"}"></a><div class="product-body"><span class="product-category">${p.category||"Ürün"}</span><h3><a href="/detay.html?tip=urun&slug=${encodeURIComponent(p.slug||seoSlug(p.name))}">${p.name||""}</a></h3><ul class="product-features">${productFeatures(p).map(f=>`<li>${f}</li>`).join("")}</ul><div class="product-footer"><a class="product-whatsapp-button" href="${whatsapp(DATA.phoneLink,`${p.name||"Ürün"} için fiyat bilgisi almak istiyorum.`)}" target="_blank" rel="noopener">Fiyat Sor</a></div></div></article>`).join("");
...
function renderWorks(items=[]){
 ...
 const draw=cat=>{const list=cat==="Tümü"?items:items.filter(x=>(x.category||"Diğer")===cat);root.innerHTML=list.map((w,i)=>{const photos=normalizePhotos(w);return `<article class="work-card reveal"><a class="work-cover" href="/detay.html?tip=is&slug=${encodeURIComponent(w.slug||seoSlug(w.title))}"><img src="${photos[0]||"/armek-logo.jpg"}" alt="${w.title||"Yaptığımız iş"}">${photos.length>1?`<span class="photo-count">▣ ${photos.length} fotoğraf</span>`:""}</a><div class="work-body"><h3><a href="/detay.html?tip=is&slug=${encodeURIComponent(w.slug||seoSlug(w.title))}">${w.title||""}</a></h3><div class="work-meta"><span>${w.category||""}</span>${w.location?`<span>📍 ${w.location}</span>`:""}${w.date?`<span>🗓 ${w.date}</span>`:""}</div>${w.text?`<p>${w.text}</p>`:""}</div></article>`}).join("");$$("[data-work-index]").forEach(el=>el.onclick=()=>openLightbox(items[Number(el.dataset.workIndex)]));observeReveals()};
